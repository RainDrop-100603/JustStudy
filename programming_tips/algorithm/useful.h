...
// num까지의 숫자들의 소수 여부를 구하고 싶다면, 에라스토테세의 채를 먼저 구한 후, isPrime(num,eratosthenes)함수를 이용한다. 소수를 단 한번만 구하면
// 된다면, isPrime(num)을 이용한다. (자동으로 eratosthenes를 구해준다.). 여러개의 소수를 확인할 때는 비효율적이다.
...
bool useful_isPrime(int num, const vector<unsigned char>& eratosthenes = {});  // num이 소수인지 확인한다. eratosthenes는 기본적으로 비워둔다.
...
struct useful_Comparator {  // suffixArr에서의 정렬에 사용한다. 각 접미사가 t만큼 비교되어 있을 때, 2t만큼 비교한다.
...
vector<int> useful_getSuffixArr(string& str);  // result[order] = i, str[i:] 를 사전순으로 반환하는 접미사 배열
