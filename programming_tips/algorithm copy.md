...
>	* 문자열은 공백을 만나기 전까지만 입력됨을 유의하자(공백까지 포함하여 받는 방법도 존재는 한다.)
...
>	```
>	- 음이 아닌 정수 m과 n 
>	- m이 커지면 A(m,n)이 급격하게 증가한다. (↑는 Knuth's up-arrow notation)
>		A(0,n) = n+1
>		A(1,n) = 2+(n+3)-3
>		A(2,n) = 2*(n+3)-3
>		A(3,n) = 2↑(n+3)-3 = 2*2*...*2 - 3 = 2^(n+3)-3, (n+3번 곱셈의 반복)
>		A(4,n) = 2↑↑(n+3)-3 = 2↑2↑...↑2 -3 = 2^2^...^2 -3, (n+3번 거듭제곱(↑)의 반복)
>		A(5,n) = 2↑↑↑(n+3)-3 = 2↑↑2↑↑...↑↑2 -3 (n+3번 ↑↑의 반복)
>	```
...
# 트라이(trie)
- 문자열은 정수, 실수 등 다른 자료구조와는 다른 탐색 방법이 필요하다.
	- M = 문자열 길이, N = 문자열 개수, sum(M) = 모든 입력 문자열의 길이의 합, R = 입력 문자열의 범위의 크기 (알파벳 소문자면 R=26)
	- 기존 방식은 탐색에 O(MlgN)만큼 필요하다. 
- 정의 및 특성 
	- 문자열의 집합을 표현하는 tree 자료구조다.
	- tree의 node는, 문자열의 접두사들에 대응된다.
		ex) TEN은 T - E - N 으로, TEX 는 T - E - X 로 대응된다.
	- find와 insert가 매우 빠르다. time complexity = O(M)
	- 메모리가 엄청나게 많이 필요하다. mem complexity = O(R*sum(M))
		- 문자열이 많지 않아서 sum(M)이 작을때 이용 가능하다. 
- 구조
	- 각 node의 값은, 가능한 접두사 중 하나이다. (알파벳 소문자로 구성되어 있다면, node의 값의 범위 = a~z)
	- 각 node는 child배열을 갖는다.
		- child배열의 길이는 node값의 범위이다. (알파벳 소문자의 경우 길이는 26)
		- child가 존재하면 pointer, 존재하지 않으면 NULL 처리한다.
		- 메모리를 크게 차지하지만 탐색을 빠르게 할 수 있게 해준다.
	- 각 node는 종료 node인지 여부를 나타내는 변수를 갖고있다.
		- 만약 해당 node가 종료 node라면, 해당 node로 끝나는 문자열이 존재한다는 것.
			ex) T - E - N 으로 탐색을 했는데, N이 종료노드라면 TEN문자열은 존재하는것이다.
- 기타 특징
	- 특정 접두사까지 find가 가능했다면, 해당 문자열을 접두사로 하는 문자열이 있음을 알 수 있다.
		- TE까지 find를 성공했다면, TE를 접두사로 갖는 문자열이 존재했음을 알 수 있다.
	- 모든 접미사를 trie에 저장하는 접미사 trie
		- 접미사를 찾는데 매우 유용하다.
		- trie의 특성에 따라, 접미사의 모든 접두사 또한 trie에 저장된다.
		- 결과적으로, 문자열의 모든 부분 문자열이 trie에 존재하게 되고, 빠르게 find할 수 있다.
		- 메모리를 엄청나게 많이 사용한다.
	- 접미사 tree	
		- 접미사를 찾는데 매우 유용하다.
		- 접미사 trie를 압축한 형태이다
			- 간선에 대응하는것이 한개의 문자가 아닌, 문자열이 될 수 있도록 하였다. (T - EN - T 와 같이 문자열 대응)
			- trie와 비교했을때, 분기가 없는 일자 node들이 압축되었다.
		- 메모리를 O(n)만큼만 이용한다
		- 구성하는데 단순한 알고리즘을 사용하면 O(n^2)이 필요하며, 효율적인 알고리즘은 너무 복잡하다.
			- 따라서 보통 접미사 tree대신 접미사 배열을 이용한다. 
