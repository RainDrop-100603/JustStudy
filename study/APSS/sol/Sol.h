...
bool Wildcard_match2(string& wildcard, string& file, int w_idx, int f_idx,
                     vector<vector<char>>& DP);  // match에서 develop
...
// AsymTiling, DP 메모제이션, 비대칭타일을 세는 것이 아닌, 전체타일 - 대칭타일 (비대칭 타일도 DP로
// 해결 가능하지만 전자가 더 간단.)
...
double Numb3rs_DP(vector<vector<int>>& townGraph, vector<int>& degree, vector<vector<double>>& DP, int dayPast,
                  int town);
...
int Packing_DP(vector<vector<int>>& DP_desp, vector<int>& itemWeight, vector<int>& itemDesp, int nowChoice,
               int weightRemain);
vector<int> Packing_Algo(int weight, vector<int>& itemWeight, vector<int>& itemDesp);
void Packing();

//@*@*@* Ocr, DP최적화,은닉 마르코프 모델,viterbi 알고리즘, P(A|B)=P(B|A)P(A)/P(B) 와 같은 변환을
//유용하게 이용하자, string 처리할 때 유의하여 처리하자,
void Ocr_Input(int& wordNum, int& sentenceNum, vector<string>& wordArr, map<string, int>& wordArrMap,
               vector<double>& firstPoss, vector<vector<double>>& nextPoss, vector<vector<double>>& classifiPoss,
               vector<string>& sentenceArr);
void Ocr_Input_test(int& wordNum, int& sentenceNum, vector<string>& wordArr, map<string, int>& wordArrMap,
                    vector<double>& firstPoss, vector<vector<double>>& nextPoss, vector<vector<double>>& classifiPoss,
                    vector<string>& sentenceArr);
void Ocr_randInput(int& wordNum, int& sentenceNum, vector<string>& wordArr, map<string, int>& wordArrMap,
                   vector<double>& firstPoss, vector<vector<double>>& nextPoss, vector<vector<double>>& classifiPoss,
                   vector<string>& sentenceArr);
void Ocr_input_simpleT(int& wordNum, int& sentenceNum, vector<string>& wordArr, map<string, int>& wordArrMap,
                       vector<double>& firstPoss, vector<vector<double>>& nextPoss,
                       vector<vector<double>>& classifiPoss, vector<string>& sentenceArr);
double Ocr1_DP1(vector<vector<double>>& DP_Ocr1, vector<vector<double>>& nextPoss, vector<vector<double>>& classifiPoss,
                int now, int nextGuess);
double Ocr1_DPposs(vector<vector<double>>& DP_Ocr1, vector<double>& firstPoss, vector<vector<double>>& nextPoss,
                   vector<vector<double>>& classifiPoss, vector<vector<double>>& DP_Poss, vector<vector<int>>& DP_Path,
                   vector<int>& wordOfSentence, int idx, int nowWord);
vector<string> Ocr1_Algo(int& wordNum, int& sentenceNum, vector<string>& wordArr, map<string, int>& wordArrMap,
                         vector<double>& firstPoss, vector<vector<double>>& nextPoss,
                         vector<vector<double>>& classifiPoss, vector<string>& sentenceArr);
double Ocr2_possQ(const vector<vector<double>>& DP, int prev, int now);
double Ocr2_DPposs(const vector<vector<double>>& DP_possQ, const vector<vector<double>>& DP_RgivenQ,
                   vector<vector<double>>& DP_Poss, vector<vector<int>>& DP_Path, const vector<int>& wordOfSentence,
                   int nextIdx, int nowWord);
vector<string> Ocr2_Algo(int wordNum, int sentenceNum, const vector<string>& wordArr,
                         const map<string, int>& wordArrMap, const vector<double>& firstPoss,
                         const vector<vector<double>>& nextPoss, const vector<vector<double>>& classifiPoss,
                         const vector<string>& sentenceArr);
void Ocr();
...
vector<int> KLIS_kthLIS(vector<vector<pair<int, int>>>& history, vector<int>& cache_numOfCases, int LISidx, int orderK,
...
// @*@*@* 지나친 수학적 접근이 문제, 프로그래밍적으로 접근필요, overflow의 다양한 처리법, 함수내에서
// 한번에 하나의 행동만
// skip번째 ele를 구하는 프로그램을 len번 반복하게 만들어도 큰 무리가 없다. 오히려 내 방법보다 더
// 간단할 수도 있음(내 방법은 substr을 많이 사용함)
void Dragon_example();
...
// @*@* 조건을 이용하여 연산을 줄인다. 특정 순열보다 작은 순열에 대한 계산. history의 필요유무에
// 따라 다른 DP, 숫자는 arr보다는 str
...
// @* sliding window, algo와 algo2의 속도차이에 유의하자(하드웨어적 원인). 속도가 느린 장치에 자주
// 접근할수록, 속도는 느려진다.
...
void GENIUS_Input(int& songNum, int& targetTime, int& favSongNum, vector<int>& songPlaytime, vector<int>& favSongList,
...
// @* greedy, atan2 함수, 원에서의 각도를 이용할땐, 2pi마다 반복되는것을 고려하고 정규화, 자료형 잘
// 확인하기
...
// combination, 답이 맞는지부터 확인, 시간조건을 맞추기 위해 최적화 하나씩 추가하면서 확인(한번에
// 추가 x), 함수가 너무 많다
...
bool BOARDCOVER2_board_can_input(const vector<vector<char>>& board, const vector<vector<char>>& block, int row,
                                 int col);
...
int BOARDCOVER2_func(vector<vector<char>>& board, const vector<vector<vector<char>>>& block_arr, int& tmp_max,
                     pair<int, int> heuristic1, int prev_value, int now_idx);
...
vector<pair<int, long long>> ALLERGY_food_opti(const vector<string>& friendsName,
                                               const vector<vector<string>>& foodsInfo);
void ALLERGY_func(const vector<pair<int, long long>>& foods_bitmask, int friendsNum, long long friends_bitmask,
                  int& tmp_min, int prev_count, int food_idx);
...
pair<int, int> KAKURO2_findHint(const vector<vector<int>>& board, const vector<vector<int>>& hint,
                                const vector<vector<int>>& hint_remain_arr,
                                const vector<vector<pair<int, int>>> board_hint);
...
void KAKURO2_set(vector<vector<int>>& board, vector<vector<int>>& hint, vector<vector<int>>& hint_remain_arr,
                 pair<int, int> twoHint, int input, int mode);
bool KAKURO2_func(vector<vector<int>>& board, vector<vector<int>>& hint, vector<vector<int>>& hint_remain_arr,
                  const vector<vector<pair<int, int>>> board_hint);
...
bool KAKURO2_func2(vector<vector<int>>& board, vector<vector<int>>& hint, vector<vector<vector<int>>>& cache,
                   const vector<vector<pair<int, int>>> board_hint);
...
double DARPA_func(int cameraNum, const vector<double>& cameraDistance, const vector<double>& cameraPoint, int left,
                  int right);
...
bool WITHDRAWL_decision(int classNum, int requiredNum, vector<int>& order, vector<int>& students, double left,
                        double right);
...
/*
  삼분탐색
    달리기거리(r)에 따른 시간(time)그래프, 시간이 낮을수록 좋은것
      ...
      time_min(정상참가자)의 그래프는 위로 볼록한 2차함수 모양이다(모양만 그렇다는것, concave
  function) 따라서 정상참가자 그래프-부정행위자 그래프는 위로 볼록한 2차함수 모양이다. local
  maximun이 존재하고, 남은 구간에서 strictly decresing(increasing) 하므로 삼분탐색 이용가능 삼분탐색
  함수 f(r) = min(정상참가자 time) - 부정행위자 time 개선점 사실 삼분탐색 조건을 완전히 만족하는
  것이 아니다. 두 그래프의 기울기가 일치하는 지점이 있을 수 있기 때문이다. 두 그래프의 기울기가
  일치하면, 합친 그래프의 기울기는 0이 되는것을 의미한다. 그러나 기울기가 0이 되는 부분은 반드시
  최대(최소)이기 때문에, 여전히 삼분탐색을 이용할 수 있다.
*/

// @* 수치해석, 삼분탐색, 차분히 힌트를 찾아서 풀자
void FOSSIL_Input(int& fig1Num, int& fig2Num, vector<pair<double, double>>& figure1,
                  vector<pair<double, double>>& figure2);
...
double FOSSIL_Algo(int fig1Num, int fig2Num, vector<pair<double, double>> figure1,
                   vector<pair<double, double>> figure2);
void FOSSIL();

// @* 정수론, 약수계산, 에라스토테네스의채, DP와 전역변수등을 적절히 이용하자, brute force도
// 고려할만 하다.
...
void PINBALL_Input(int& xPos, int& yPos, int& dx, int& dy, int& num, vector<int>& centerX, vector<int>& centerY,
                   vector<int>& radius);
vector2 PINBALL_lineSym(vector2 point1, vector2 a, vector2 b);
pair<int, vector2> PINBALL_ifMet(vector<int>& centerX, vector<int>& centerY, vector<int>& radius, vector2 ballPoint,
                                 vector2 ballVector);
vector<int> PINBALL_func(vector<int>& centerX, vector<int>& centerY, vector<int>& radius, vector2 ballPoint,
                         vector2 ballVector, int count);
vector<int> PINBALL_Algo(int xPos, int yPos, int dx, int dy, int num, vector<int> centerX, vector<int> centerY,
                         vector<int> radius);
void PINBALL();

// @*@*@* 계산 기하, simple polygon clipping, 배열 접근시, 배열의 크기가 0인 경우도 반드시 생각하자
//        예외가 많아보이는 방법이라면, 모든 예외를 처리하는것도 좋지만, 더 간단한 방법은 없는지
//        생각해보자
...
// vector<vector2> TREASURE_Fail_outsidePoly(vector<vector2>& polygon,vector<vector2> treasure,int
// begin,int end); int TREASURE_Fail_polyOverlap(const vector<vector2>& poly1, const
// vector<vector2>& poly2); double TREASURE_Fail_Algo(vector<vector2> polygon, vector<vector2>
// treasure);
...
// @*@*@* 계산 기하, 여러개의 점으로 convex hull 만드는법, convex hull을 나누느 직선이 있는지 구하는
// 법
