...
function displayElement(name,key,value){
  ...
  //make Btn
  if(name==="TDL-Pending"){
    tmpLi.appendChild(makeBtn("✔","TDL-Finished"));
  }
  if(name==="TDL-Finished"){
    tmpLi.appendChild(makeBtn("♻","TDL-Pending"));
  }
...
function onlyForInput(){
  const form=TDL_Container.querySelector("form");
  const input=form.querySelector("input");
  form.addEventListener("submit",function(event){
    event.preventDefault();
    const parentName="TDL-Pending";
    const inputValue=event.target.querySelector("input").value;
    event.target.querySelector("input").value="";
    //add element to list
    const ID=addElement(parentName,inputValue);
