...
SO_URL="https://stackoverflow.com/jobs?q="
SO_VIEWJOB="https://stackoverflow.com"
...
def get_maxpage(keyward):
  result=requests.get(f"{SO_URL}{keyward}")
...
def get_jobs(keyward,page):
  req=requests.get(f"{SO_URL}{keyward}&pg={page+1}")
  ...
  for jobcard in jobcard_list:
    ...
    job={
      ...
      "link":SO_VIEWJOB+jobcard.find("h2").find("a")["href"]
    }
    job_list.append(job)
  return job_list
