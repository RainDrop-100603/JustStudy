...
??	Double Pointer (Multi Pointer)
		...
		만약 한번만 참조를 하면 어떻게 되는가? 
				빌드 에러가 발생한다. (초기화 할 수 없다고 나온다) -> 초기화 할 수 없는 이유는 Meaning of '&' and '*'에서 확인 
			*****Meaning of '&' and '*' 참고하자
...
/*Meaning of '&' and '*'
	...
	그 외 연산시
		&는 객체(변수)의 공간(위치)를 나타낸다.
		*은 포인터가 가리키는 공간을 참조(반환?)하는 것이다(주소의 위치에 있는 객체(변수)를 나타낸다.)
			*이 여러개라면 여러번 참조하는 것이다.
	대입 연산자 의 이용
		포인터에는 값을 대입할 수 없다.
			포인터는 변수의 자료형이 포인터라는 것이다. 즉 그 자체는 공간이 아닌 변수이다.
		공간에는 값을 대입할 수 있다.
			Reference는 변수가 저장된 공간(위치)를 나타낸다. 즉 그 자체는 변수가 아닌 공간이다.
